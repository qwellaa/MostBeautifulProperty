...
public class DesignAdapter extends BaseAdapter {
    ...
    private void showShare(int position) {
        ...
        oks.setTitle(mDesignerBean.getList().get(position).getShare_url());
        // titleUrl是标题的网络链接，QQ和QQ空间等使用
        oks.setTitleUrl(mDesignerBean.getList().get(position).getShare_url());
...
//        oks.setImagePath("/sdcard/test.jpg");//确保SDcard下面存在此张图片
        ...
        oks.setSiteUrl(mDesignerBean.getList().get(position).getShare_url());
